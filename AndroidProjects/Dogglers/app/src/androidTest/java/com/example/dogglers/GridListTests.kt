...
class GridListTests : BaseTest() {
    ...
    fun `grid_list_content_on_first_page`() {
        onView(withText(dogs.first().name)).check(matches(isDisplayed()))
    }
    ...
    fun `grid_list_content_at_last_position`() {
        ...
        onView(withText(dogs.last().name)).check(matches(isDisplayed()))
    }
    ...
    fun `vertical_scrolling`() {
        onView(withId(R.id.grid_recycler_view))
            .perform(swipeUp())
        onView(withText(dogs[5].name)).check(matches(isDisplayed()))
    }

    @Test
    fun `recycler_view_item_count`() {
        onView(withId(R.id.grid_recycler_view)).check(hasItemCount(dogs.size))
    }
}
